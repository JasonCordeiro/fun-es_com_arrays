...
function retornaValorEpecifico(valor) {
    for (let panda = 0; panda < listaDeElementos.length; panda++) {
        if (panda == valor) {
            return `O valor contido nesta posição é:" ${listaDeElementos[panda]}` 
        }
    }
}

//console.log(retornaValorEpecifico())
...
function retornarTipoElemento(valor) {
    for (let panda = 0; panda < listaDeElementos.length; panda++){
        if(panda == valor) {
            if (typeof listaDeElementos[panda] == "string") {
                alert(`O elemento ${listaDeElementos[panda]} é um elemento do tipo texto.`)
            }
            else{ 
                alert(`O elemento ${listaDeElementos[panda]} é um elemento do tipo numero.`)
            }
        }
    }
}
...
function removeUltimoElementoString () {
    if (typeof listaDeElementos[listaDeElementos.length -1] == 'string') {
        ...
        return 'Falha ao remover o elento da lista!';
    }
}
...
function consultaPosição(valor) {
    for (let panda = 0; panda < listaDeElementos.length; panda++) {
        if (listaDeElementos[panda] == valor) {
            return `O valor procurado esta na posição: ${panda}`
        }
    }
    return "Valor não encontrado!"
}

//console.log(consultaPosição())
...
//console.log(listaDeElementos)
//console.log(removerElementoEspecifico)
//console.log(listaDeElementos)
